defmodule MegalithicWeb.PageLive do
  use MegalithicWeb, :live_view

  @impl true
  def mount(_params, _session, socket) do
    {:ok, assign(socket, :page_title, socket.assigns.live_action |> to_string() |> String.capitalize())}

    # if socket.assigns.live_action == :home do
    #   {:ok, redirect(socket, to: Routes.blog_path(socket, :index))}
    # else
    #   title = socket.assigns.live_action |> to_string() |> String.capitalize()
    #   {:ok, assign(socket, :page_title, title)}
    # end
  end

  @impl true
  def render(assigns) do
    MegalithicWeb.PageView.render("#{assigns.live_action}.html", assigns)

    # if assigns.live_action == :home do
    #   ""
    # else
    #   MegalithicWeb.PageView.render("#{assigns.live_action}.html", assigns)
    # end
  end
end
